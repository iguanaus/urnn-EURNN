...
def EURNN2(n_input,n_hidden,n_output,out_every_t=False,capacity=1,approx=False,loss_function="CE"):
    ...
    permutedList1, permutedList2 = permute_list12(n_hidden)
    permutedList3, permutedList4 = permute_list34(n_hidden)

    if approx:
        ind_list5, ind_list6 = permute_approx(n_output)

    permutedHidden = []
    for i in xrange(0,n_hidden):
      if i % 2 == 0:
        permutedHidden.append(i+1)
      else:
        permutedHidden.append(i-1)
    ...
    if approx:
        HArray = np.asarray(rng.uniform(low=-bin,
                high=bin,
                size=(int(np.log2(n_hidden)), int(n_hidden/2))),
                dtype=theano.config.floatX)
        HTensor = theano.shared(value=HArray,name="H Matrix", broadcastable=(False,False))
    else:
        HArray = np.asarray(rng.uniform(low=-bin,
                high=bin,
                size=(int((capacity+1)/2), int(n_hidden/2))),
                dtype=theano.config.floatX)
        HTensor = theano.shared(value=HArray,name="H Matrix", broadcastable=(False,False))

    HTensor_2 = theano.shared(value=HArray)
    if (capacity >1 and not approx):

        HArray_2 = np.asarray(rng.uniform(low=-bin,
                high=bin,
                size=(int((capacity)/2), int(n_hidden/2)-1)),
                dtype=theano.config.floatX)
        
        HTensor_2 = theano.shared(value=HArray_2,name="H 2 Matrix", broadcastable=(False,False))
    ...
    def recurrence(x_t,y_t,h_prev,cost_prev,acc_prev,U,W,V,W_2=None):
        ...
        cos_list = T.concatenate([cos_theta,cos_theta],axis=1)
        sin_theta = T.sin(W)#.dimshuffle('x',0)
        sin_list = T.concatenate([sin_theta,-sin_theta],axis=1)
        ...
        if capacity ==1:
            ...
            cos_theta_1 = T.cos(W_2)#.dimshuffle('x',0)
            cos_list_1 = T.concatenate([np.ones((int(capacity/2),1),dtype=np.float32),cos_theta_1,cos_theta_1,np.ones((int(capacity/2),1),dtype=np.float32)],axis=1)


            sin_theta_1 = T.sin(W_2)#.dimshuffle('x',0)
            sin_list_1 = T.concatenate([np.zeros((int(capacity/2),1),dtype=np.float32),sin_theta_1,-sin_theta_1,np.zeros((int(capacity/2),1),dtype=np.float32)],axis=1)


            diag_list_1 = cos_list_1[:,permutedList4]
            off_list_1 = sin_list_1[:,permutedList4]

        #Now we for loop it.

        if approx:
            print("EURNN: Using approximate values")
            state = h_prev
            for i in xrange(0,int(np.log2(n_output))):
                d0 = diag_list_0[i]
                o0 = off_list_0[i]
                ind5 = ind_list5[i]
                ind6 = ind_list6[i]
                ...
                step1 = state*permuted
                step2 = (state*o0[:,ind6])[:,ind5]
                state = step1 + step2
        else:
            if (capacity==1):
                state = h_prev
                ...
                for i in xrange(0,int(capacity/2)):
                    #print("For looping!!!!")
                    d0 = diag_list_0[i]
                    o0 = off_list_0[i]
                    d1 = diag_list_1[i]
                    o1 = off_list_1[i]
                    step1 = state*d0+state[:,permutedList1]*o0
                    state = step1*d1+step1[:,permutedList2]*o1
                    #print('State: ' , state)
                if (capacity %2 ==1):
                    #One last rotation
                    d = diag_list_0[capacity/2]
                    o = off_list_0[capacity/2]
                    state = state*d+state[:,permutedList1]*o
        #Okay state should have it
